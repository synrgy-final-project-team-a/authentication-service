...
//        }
//        map = userService.registerAdmin(objModel);
////        Map sendOTP = sendEmailegister(objModel);
//        Map sendOTPUri = sendEmailRegisterTymeleafAdmin(objModel);
//
...
//    int expiredToken;
//
//    // Step 2: send OTP berupa URL: guna updeta enable agar bisa login:
////    @GetMapping(path = "/send-otp")//send OTP
////    public Map sendEmailegisterTymeleafAdmin(@RequestBody RegisterModel user) {
...
////            template = template.replaceAll("\\{\\{USERNAME}}", (found.getEmail() == null ? found.getUsername() : found.getEmail()));
...
//    public Map sendEmailRegisterTymeleafAdmin(@RequestBody RegisterModel user) {
//        String message = "Thanks, please check your email for activation.";
//
//        if (user.getEmail() == null) return response.templateEror("No email provided");
//        User found = userRepository.findOneByEmail(user.getEmail());
...
//            template = template.replaceAll("\\{\\{VERIFY_TOKEN}}", BASEURL + "/admin-register/index/" + otp);
//            userRepository.save(found);
//
//        } else {
//            template = template.replaceAll("\\{\\{USERNAME}}", (found.getUsername() == null ? found.getEmail() : found.getUsername()));
//            template = template.replaceAll("\\{\\{VERIFY_TOKEN}}", BASEURL + "/admin-register/index/" + found.getOtp());
//        }
