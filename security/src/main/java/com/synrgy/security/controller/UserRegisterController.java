...
//        }
//        map = userService.registerUser(objModel);
...
////    @PostMapping(path = "/send-otp")//send OTP
...
//            template = template.replaceAll("\\{\\{VERIFY_TOKEN}}", BASEURL + "/user-register/index/"+ otp);
//            userRepository.save(found);
//        } else {
//            template = template.replaceAll("\\{\\{USERNAME}}", (found.getUsername() == null ? found.getEmail() : found.getUsername()));
//            template = template.replaceAll("\\{\\{VERIFY_TOKEN}}", BASEURL + "/user-register/index/"+  found.getOtp());
//        }
