...
public interface UserRepository extends PagingAndSortingRepository<User, Long> {
    @Query(value = "SELECT * FROM oauth_user AS usr JOIN profile AS prf ON usr.profile_id = prf.id WHERE LOWER(usr.email) = LOWER(?1) AND usr.deleted_at IS NULL", nativeQuery = true)
